...
public class ChromeDriverUtils {
	...
	public static List<Map<Integer, String>> graspStockInfo(String chromeDriverPath, String tdccQryStockUrl,
        ...
        try {
            ...
            for (String tdccSelectoption : tdccSelectoptions) {
                ...
                WebElement searchButton = wait.until(ExpectedConditions.presenceOfElementLocated(
                        By.xpath("//input[@type='submit' and @value='查詢']")));
                searchButton.click();
                WebElement tbodyElement = wait.until(ExpectedConditions.visibilityOfElementLocated(
                        By.cssSelector(".table-frame.securities-overview.m-t-20 tbody")));
                ...
                WebElement selectElement = wait.until(
                        ExpectedConditions.presenceOfElementLocated(By.name("month")));
                Select monthSelect = new Select(selectElement);
                monthSelect.selectByValue(StringUtils.leftPad(String.valueOf(month), 2, "0"));
                Thread.sleep(200);
                WebElement stockNoInput = wait.until(ExpectedConditions.presenceOfElementLocated(
                        By.xpath("//input[@name='kind' and @maxlength='6' and @size='8']")));
